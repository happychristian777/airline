...
<div class="container" style="margin-top: -15px;">
  <h1 class="text-center m-3">Add STAFF</h1>
  <div class="row justify-content-md-center">
    <div class="col col-md-8 col-lg-6">
   <form method="post" action="<?php echo base_url();?>Admin/upd_staff">
   <?php foreach($res as $row ) { ?>
        <div class="form-group">
        <input type="hidden" name="id" value="<?php echo $this->input->get('id')?>">
          <label for="productName">SURNAME</label>
          <input type="text" name="surname" value="<?php echo $row->surname; ?>" class="form-control" id="productName" required>
        </div>
        <div class="form-group">
          <label for="productPrice">NAME</label>
          <input name="name" type="text" class="form-control" value="<?php echo $row->name; ?>" id="productPrice" required>
        </div>
        <div class="form-group">
          <label for="productPrice">ADDRESS</label>
          <input name="address" type="text" class="form-control" value="<?php echo $row->address; ?>" id="productPrice" required>
        </div>
        <div class="form-group">
          <label for="productPrice">PHONE NUMBER</label>
          <input type="text" name="phone" class="form-control" value="<?php echo $row->phone; ?>" id="productPrice" required>
        </div>
        <div class="form-group">
          <label for="productPrice">SALARY</label>
          <input type="text" name="salary" class="form-control" value="<?php echo $row->salary; ?>" id="productPrice" placeholder="e.g. $1000" required>
        </div>
   <?php } ?>
